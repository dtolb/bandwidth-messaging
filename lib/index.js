...
function Client(config) {
  ...
  const findError = res => {
    ...
    switch (status) {
      case 400:
        return new be.BadRequestError(status);
      case 401:
        return new be.UnauthorizedError(status);
      case 403:
        return new be.ForbiddenError(status);
      case 415:
        return new be.UnsupportedContentTypeError(status);
      case 429:
        return new be.RateLimitError(status);
      default:
        return new be.UnexpectedResponseError(status);
    }
  }
